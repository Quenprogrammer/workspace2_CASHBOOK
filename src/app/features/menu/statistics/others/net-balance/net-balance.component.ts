...
@Component({
  ...
  styleUrl: './net-balance.component.css'
})
export class NetBalanceComponent {
  ...
  drawChart() {
    const canvas = document.getElementById('netBalance') as HTMLCanvasElement;
    if (!canvas) return;

    const ctx = canvas.getContext('2d');
    if (!ctx) return;
    const data = [0, 100, 200, 175, 100, 50, 75, 0, 0, 50, 50, 50, 0 ];
    const labels = ['D1', ' ', 'D3', 'D15']; const maxWidth = canvas.width;
