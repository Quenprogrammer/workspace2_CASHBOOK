...
@Component({
  selector: 'app-expenses-transactions',
  standalone: true,
  imports: [
    HeadingComponent
  ],
  templateUrl: './expenses-transactions.component.html',
  styleUrl: './expenses-transactions.component.css'
})
export class ExpensesTransactionsComponent {
  ngAfterViewInit() {
    this.drawChart();
  }

  drawChart() {
    const canvas = document.getElementById('expenses') as HTMLCanvasElement;
    if (!canvas) return;
